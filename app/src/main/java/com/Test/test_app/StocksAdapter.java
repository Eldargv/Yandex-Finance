...
public class StocksAdapter extends RecyclerView.Adapter<StocksAdapter.StockViewHolder> {
    ...
    public void onBindViewHolder(@NonNull StockViewHolder holder, int position) {
        CompanyModel company = StockList.get(position);
        int ResId = R.drawable.background_shape;
        if (position % 2 == 1) {
            ResId = R.drawable.background_shape_white;
        }
        holder.bind(company.getTicker(), company.getName(), company.getLogoUrl(), company.getCurrency(),company.getCurrentPrice(), company.getDifferent(), ResId);
    }
    ...
    static class StockViewHolder extends RecyclerView.ViewHolder {
        ...
        void bind(String tick, String name, String logoUrl, String Currency, Double currentPrice, Double differentPrice, Integer resId) {
            StockTick.setText(tick);
            StockName.setText(name);
            Background.setBackgroundResource(resId);
            String currency = "";
            try {
                JSONObject currobject = jsonObject.getJSONObject(Currency);
                currency = currobject.getString("symbol");
            } catch (JSONException e) {
                e.printStackTrace();
            }
            String sign = "+";
            ...
            // Строим новый запрос на clearbit api
            String host = "";
            try {
            ...
            imageLoader.loadImage(realUrl, new SimpleImageLoadingListener() {
