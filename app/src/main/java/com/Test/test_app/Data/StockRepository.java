...
public class StockRepository {
    ...
    public StockRepository() {
        ...
        defaultProgress.setValue(0);
    }

    public void insert(Stock stock) {
        Log.i("TAG", "Try to insert a new stock in the thread");
        new InsertThread(stock, stockDao).start();
    }

    public void update(Stock stock) {
        new UpdateThread(stock, stockDao).start();
    }

    public void delete(Stock stock) {
        new DeleteThread(stock, stockDao).start();
    }
    ...
    private static class InsertThread extends Thread {
        ...
        public InsertThread(Stock stock, StockDao stockDao) {
            this.stock = stock;
    ...
    private static class DeleteThread extends Thread {
        ...
        public DeleteThread(Stock stock, StockDao stockDao) {
            this.stock = stock;
            this.stockDao = stockDao;
        }
    ...
    private static class UpdateThread extends Thread {
        ...
        public UpdateThread(Stock stock, StockDao stockDao) {
            this.stock = stock;
            this.stockDao = stockDao;
        }
    ...
    public MutableLiveData<Integer> getDefaultProgress() {
        ...
        call.enqueue(new Callback<ConstituentModel>() {
            ...
            public void onResponse(Call<ConstituentModel> call, Response<ConstituentModel> response) {
                ...
                for (int i = 0; i < Math.min(constituents.size(), 15); i++) {
                    getProfile(constituents.get(i), defaultList);
                }
            }
    ...
    public void SearchQuery(String query) {
        stockRunnable = new StockRunnable(query);
        searchThread = new Thread(stockRunnable);
        searchThread.start();
    }
            ...
            try {
                ...
                for (int i = 0; i < response.body().getCount(); i++) {
                    ...
                    if (res.getType().equals("Common Stock")) {
                        ...
                        getProfile(res.getSymbol(), searchList);
                        Thread.sleep(200);
                    }
                }
    ...
    void getProfile(String symbol, MutableLiveData<List<Stock>> LiveStockList) {
...
//                    }
                ...
                if (response.body().getTicker() == null || response.body().getTicker().equals("")) {
                    return;
                }
                ...
                getCurrDiff(symbol, stock, LiveStockList);
            }
    ...
    void getCurrDiff(String symbol, Stock stock, MutableLiveData<List<Stock>> LiveStockList) {
...
//                        Toast.makeText(getActivity(), "API limit reached. Please try again later", Toast.LENGTH_LONG).show();
                ...
                if (favoriteList.getValue() != null) {
                    ...
                    if (!favoriteList.getValue().contains(stock)) {
                ...
                defaultProgress.setValue(100 * LiveStockList.getValue().size() / 15);
                Log.i("TAG", "default progress " + 100 * LiveStockList.getValue().size() / 14);
            }
